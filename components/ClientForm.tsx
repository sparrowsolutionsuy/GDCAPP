...
export const ClientForm: React.FC<ClientFormProps> = ({ onAddClient }) => {
  // Allow latitud and longitud to be string or number during editing
  const [formData, setFormData] = useState<Partial<Omit<Client, 'latitud' | 'longitud'> & { latitud: string | number, longitud: string | number }>>({
  ...
  const handleRutChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    // Ensure only numbers and max 12 digits
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.nombreComercial && formData.latitud && formData.longitud && formData.rut) {
      ...
      // Basic RUT validation length check (optional, can be strict 12)
