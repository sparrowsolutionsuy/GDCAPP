        ...
        <div className="flex-1 overflow-y-auto">
          {filteredClients.map(client => (
            <div 
              ...
              className={`p-4 border-b border-slate-100 cursor-pointer transition-colors hover:bg-slate-50 ${selectedClient?.id === client.id ? 'bg-blue-50 border-l-4 border-l-blue-600' : ''}`}
            >
              <div className="flex justify-between items-start">
                  <h3 className="font-semibold text-slate-800">{client.nombreComercial}</h3>
                  {client.rut && <span className="text-[10px] bg-slate-200 text-slate-600 px-1.5 py-0.5 rounded">RUT: {client.rut}</span>}
              </div>
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <div className="flex items-center text-sm text-slate-500 mt-1 font-mono">
                      <Hash className="w-3 h-3 mr-1" /> RUT: {selectedClient.rut}
                  </div>
                  
                  <div className="flex flex-wrap items-center gap-4 mt-3 text-sm text-slate-600">
                    <span className="flex items-center"><MapPin className="w-4 h-4 mr-1 text-slate-400"/> {selectedClient.localidad}, {selectedClient.departamento}</span>
                    <span className="flex items-center"><Mail className="w-4 h-4 mr-1 text-slate-400"/> {selectedClient.email}</span>
                    <span className="flex items-center"><Phone className="w-4 h-4 mr-1 text-slate-400"/> {selectedClient.telefono}</span>
                  </div>
                </div>
              ...
              <table className="w-full text-left text-sm text-slate-600">
                ...
                <tbody className="divide-y divide-slate-100">
                  {clientTrips.map(trip => {
                     ...
                     return (
                      <tr key={trip.id} className="hover:bg-slate-50">
                        ...
                        <td className="p-4">{trip.contenido} <span className="text-xs text-slate-400">({(trip.pesoKg/1000).toFixed(1)}t)</span></td>
                  ...
                  {clientTrips.length === 0 && (
                    <tr>
                      <td colSpan={6} className="p-8 text-center text-slate-400">No hay historial de viajes.</td>
                    </tr>
                  )}
                </tbody>
              </table>
