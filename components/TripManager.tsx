...
export const TripManager: React.FC<TripManagerProps> = ({ trips, clients, onAddTrip, user }) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (newTrip.clientId && newTrip.pesoKg) {
      // Allow 0 for tariff if user is operative (they might not know the price)
      ...
      if (editingId) {
        ...
        const trip: Trip = {
            ...
            estado: newTrip.estado || TripStatus.PROGRAMMED,
  ...
  const handleDelete = async (id: string) => {
    ...
    const confirmed = window.confirm(`ATENCIÓN: ¿Está seguro de que desea eliminar el viaje ${id}?\n\nEsta acción eliminará el registro de la hoja de cálculo y no se puede deshacer.`);
  ...
  const getClientName = (id: string) => clients.find(c => c.id === id)?.nombreComercial || 'Desconocido';

  const calculateBenefit = (trip: Trip) => {
    if (trip.kmRecorridos <= 0) return 0;
  ...
  const StatusBadge = ({ status }: { status: TripStatus }) => {
    const styles = {
      [TripStatus.IN_PROGRESS]: 'bg-yellow-100 text-yellow-800 border-yellow-200',
      [TripStatus.PROGRAMMED]: 'bg-blue-100 text-blue-800 border-blue-200',
      [TripStatus.COMPLETED]: 'bg-green-100 text-green-800 border-green-200',
      [TripStatus.CLOSED]: 'bg-slate-100 text-slate-800 border-slate-300',
    };
    return (
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        <div className="flex bg-slate-200 p-1 rounded-lg">
          <button onClick={() => setActiveTab('current')} className={`px-4 py-2 text-sm font-medium rounded-md transition-all ${activeTab === 'current' ? 'bg-white text-blue-900 shadow-sm' : 'text-slate-600'}`}>En Curso</button>
          <button onClick={() => setActiveTab('programmed')} className={`px-4 py-2 text-sm font-medium rounded-md transition-all ${activeTab === 'programmed' ? 'bg-white text-blue-900 shadow-sm' : 'text-slate-600'}`}>Programados</button>
          <button onClick={() => setActiveTab('all')} className={`px-4 py-2 text-sm font-medium rounded-md transition-all ${activeTab === 'all' ? 'bg-white text-blue-900 shadow-sm' : 'text-slate-600'}`}>Historial</button>
        </div>
        <button onClick={() => setShowForm(true)} className="bg-blue-900 hover:bg-blue-800 text-white px-4 py-2 rounded-lg flex items-center shadow-lg transition-colors">
      ...
      {showForm && (
        <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200 animate-fade-in-down">
          <div className="flex justify-between items-center mb-4 border-b pb-2">
            <h3 className="text-lg font-bold text-slate-800">{editingId ? 'Editar Viaje' : 'Registrar Nuevo Viaje'}</h3>
          ...
          <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="md:col-span-2">
              ...
              <select required className="w-full border-slate-300 rounded-lg p-2 bg-slate-50 outline-none" value={newTrip.clientId || ''} onChange={e => setNewTrip({...newTrip, clientId: e.target.value})}>
            ...
            <div>
              ...
              <input type="date" required className="w-full border-slate-300 rounded-lg p-2 bg-slate-50 outline-none border" value={newTrip.fecha} onChange={e => setNewTrip({...newTrip, fecha: e.target.value})} />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Estado</label>
              <select className="w-full border-slate-300 rounded-lg p-2 bg-slate-50 outline-none border" value={newTrip.estado} onChange={e => setNewTrip({...newTrip, estado: e.target.value as TripStatus})}>
                <option value={TripStatus.PROGRAMMED}>Programado</option>
                <option value={TripStatus.IN_PROGRESS}>En Curso</option>
                <option value={TripStatus.COMPLETED}>Finalizado</option>
                <option value={TripStatus.CLOSED}>Cerrado</option>
            ...
            <div><label className="block text-sm font-medium text-slate-700 mb-1">Producto</label><input type="text" required className="w-full border-slate-300 rounded-lg p-2 bg-slate-50 outline-none border" value={newTrip.contenido} onChange={e => setNewTrip({...newTrip, contenido: e.target.value})} /></div>
            <div><label className="block text-sm font-medium text-slate-700 mb-1">Origen</label><input type="text" required className="w-full border-slate-300 rounded-lg p-2 bg-slate-50 outline-none border" value={newTrip.origen} onChange={e => setNewTrip({...newTrip, origen: e.target.value})} /></div>
            <div><label className="block text-sm font-medium text-slate-700 mb-1">Destino</label><input type="text" required className="w-full border-slate-300 rounded-lg p-2 bg-slate-50 outline-none border" value={newTrip.destino || ''} onChange={e => setNewTrip({...newTrip, destino: e.target.value})} /></div>
            
            {/* KM Input with Smart Suggestion */}
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1 flex justify-between">
                <span>KM Recorridos</span>
                {suggestedKm && <span className="text-xs text-blue-600 flex items-center animate-pulse"><Sparkles className="w-3 h-3 mr-1"/> Historial</span>}
              </label>
              <input 
                type="number" 
                required 
                className={`w-full border rounded-lg p-2 outline-none transition-colors ${suggestedKm ? 'bg-blue-50 border-blue-300 text-blue-800 font-semibold' : 'bg-slate-50 border-slate-300'}`}
                value={newTrip.kmRecorridos || ''} 
                onChange={e => {
                  setNewTrip({...newTrip, kmRecorridos: Number(e.target.value)});
                  setSuggestedKm(false); // Remove suggestion state if user manually edits
                }} 
              />
            </div>

            <div><label className="block text-sm font-medium text-slate-700 mb-1">Peso (KG)</label><input type="number" required className="w-full border-slate-300 rounded-lg p-2 bg-slate-50 outline-none border" value={newTrip.pesoKg || ''} onChange={e => setNewTrip({...newTrip, pesoKg: Number(e.target.value)})} /></div>
            
            {isAdmin && (
              <div><label className="block text-sm font-medium text-slate-700 mb-1">Tarifa (USD/Ton)</label><input type="number" required className="w-full border-slate-300 rounded-lg p-2 bg-slate-50 outline-none border" value={newTrip.tarifa || ''} onChange={e => setNewTrip({...newTrip, tarifa: Number(e.target.value)})} /></div>
            )}

            <div className="md:col-span-2 lg:col-span-4 flex justify-end space-x-3 pt-2">
              <button type="button" onClick={closeForm} className="text-slate-500 hover:text-slate-700 font-medium">Cancelar</button>
              <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg font-medium shadow-md">
          ...
          <table className="w-full text-left text-sm text-slate-600">
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredTrips.map(trip => (
                <tr key={trip.id} className="hover:bg-slate-50 transition-colors">
                  <td className="p-4">
                    ...
                    <div className="flex items-center mt-1"><Calendar className="w-3 h-3 mr-1 text-slate-400"/> {trip.fecha}</div>
                  </td>
                  <td className="p-4"><StatusBadge status={trip.estado} /></td>
                  <td className="p-4">
                    <div className="font-medium text-slate-800">{getClientName(trip.clientId)}</div>
                    <div className="flex items-center text-xs mt-1"><Package className="w-3 h-3 mr-1 text-slate-400"/> {(trip.pesoKg/1000).toFixed(1)}t {trip.contenido}</div>
                  </td>
                  <td className="p-4">
                  ...
                  {isAdmin && (
                    <td className="p-4 text-right text-xs">
                      <div>Tarifa: <span className="font-semibold">${trip.tarifa}/t</span></div>
                      <div className="text-slate-500">Total: ${Math.round(trip.tarifa * (trip.pesoKg/1000))}</div>
                    </td>
                  )}
                  {isAdmin && (
              ...
              {filteredTrips.length === 0 && (
                <tr>
                  <td colSpan={isAdmin ? 7 : 4} className="p-8 text-center text-slate-400 italic">No se encontraron viajes.</td>
                </tr>
              )}
            </tbody>
          </table>
